...
function q(phrase: string, text: string) {
  ...
  return { full, tokens: pT.length, overlap, ratio: +ratio.toFixed(2), quality: +quality.toFixed(3) };
}
...
for (const [p, t] of rows) console.log(`  phrase="${p}"`.padEnd(40), q(p,t), " (multi≥0.6? ", (q(p,t).tokens>=2 && q(p,t).quality>=0.6) || q(p,t).full, ")");
