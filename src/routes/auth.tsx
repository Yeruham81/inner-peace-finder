...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled && data.session) {
        try { await ensureTherapistAccount(); } catch { /* non-fatal */ }
        navigate({ to: dest });
      }
    })();
  ...
  async function handleOAuth(provider: "google" | "apple") {
    ...
    if (res.error) {
      setMsg({ kind: "err", text: `כניסה דרך ${provider === "google" ? "Google" : "Apple"} נכשלה.` });
      setLoading(false);
      return;
    }
      ...
      <div className="rounded-2xl border border-border bg-surface-elevated p-6 shadow-sm">
        ...
        {tab === "signin" && (
          <form onSubmit={handleSignIn} className="grid gap-3">
            ...
            <Button type="submit" disabled={loading}>{loading ? "טוען…" : "כניסה"}</Button>
            <button type="button" className="text-xs text-muted-foreground underline" onClick={() => { setTab("forgot"); setMsg(null); }}>
              שכחתי סיסמה
            </button>
          </form>
        )}
        ...
        {tab === "signup" && (
          <form onSubmit={handleSignUp} className="grid gap-3">
            ...
            <Button type="submit" disabled={loading}>{loading ? "טוען…" : "יצירת חשבון"}</Button>
          </form>
        )}
        ...
        {tab === "forgot" && (
          <form onSubmit={handleForgot} className="grid gap-3">
            ...
            <Button type="submit" disabled={loading}>{loading ? "שולח…" : "שליחת קישור איפוס"}</Button>
            <button type="button" className="text-xs text-muted-foreground underline" onClick={() => { setTab("signin"); setMsg(null); }}>
              חזרה לכניסה
            </button>
          </form>
        )}
