 ...
 */
export const issueLeadChallenge = createServerFn({ method: "POST" }).handler(
  async (): Promise<IssuedChallenge> => {
    const {
    ...
    const retentionCutoff = new Date(Date.now() - CHALLENGE_RETENTION_MS).toISOString();
