...
describe("exact canonical semantic evidence", () => {
  it("uses complete normalized phrases only — no fuzzy token overlap", () => {
    expect(findExactCanonicalEvidence("אני מחפש עזרה עם נדודי שינה", catalog).map((x) => x.slug)).toEqual([
      "sleep_difficulties",
    ]);
  ...
  it("maps every one of the 483 curated live aliases to its canonical owner with exact evidence", () => {
    ...
    const slugById = new Map(LIVE_ACTIVE_CATALOG.problems.map((problem) => [String(problem.id), problem.slug]));
    ...
    for (const row of LIVE_ACTIVE_CATALOG.aliases) {
      ...
      expect(findExactCanonicalEvidence(row.alias, liveCatalog).map((hit) => hit.slug)).toEqual([expected]);
    }
  });
  ...
  it("prefers a longer specific alias over a nested shorter alias", () => {
    const nestedCatalog = [
      ...
      { slug: "sexual_abuse_trauma", name: "פגיעות מיניות וטראומה מינית", aliases: ["טראומה מינית"] },
    ];
    expect(findExactCanonicalEvidence("טראומה מינית", nestedCatalog).map((hit) => hit.slug)).toEqual([
      "sexual_abuse_trauma",
    ]);
  });
